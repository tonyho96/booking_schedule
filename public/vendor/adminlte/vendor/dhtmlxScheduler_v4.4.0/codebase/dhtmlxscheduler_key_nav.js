...
//Initial idea and implementation by Steve MC
    scheduler._temp_key_scope = function (){
        ...
        scheduler._make_pasted_event = function(ev){
            ...
            // Calculate the new date for the pasted booking
            copy.start_date = scheduler._calc_new_date(date, copy.start_date);
        ...
        /**
         * Gets the date from the mouse pointer, and the events time, and combines them to form the new date
         *
         * @param mouseDate
         * @param event
         * @returns {*}
         * @private
         */
        scheduler._calc_new_date = function(mouseDate, event) {
            var mouseDate = scheduler.date.date_part(mouseDate);
            var eventSeconds = scheduler.date.time_part(event);
            var eventMinutes = eventSeconds / 60;

            return scheduler.date.add(mouseDate, eventMinutes, 'minute');
        };
        ...
        scheduler._is_key_nav_active = function(){
            if(this._is_initialized() && !editMode && this.config.key_nav){
                return true;
            }
            return false;
        };
        ...
        dhtmlxEvent(document,(_isOpera?"keypress":"keydown"),function(e){
            ...
            if ((e.ctrlKey && e.keyCode == 67) || (e.metaKey && e.keyCode == 67)) {  // CTRL+C
                if (select_id) {
            ...
            if ((e.ctrlKey && e.keyCode == 88) || (e.metaKey && e.keyCode == 88)) { // CTRL+X
                if (select_id) {
                    isCopy = false;
                    scheduler._buffer_id = select_id;
            ...
            if ((e.ctrlKey && e.keyCode == 86) || (e.metaKey && e.keyCode == 86)) {  // CTRL+V
            ...
            if(e.keyCode == 46) { // DELETE
                if (select_id) {
                    dhtmlx.confirm({
                        text: "Are you sure you want to delete <u>" + scheduler.getEvent(select_id).text + "</u>?",
                        callback: function(res){
                            if(res)
                                scheduler.deleteEvent(select_id);
                        }
                    });
                }
                return true;
